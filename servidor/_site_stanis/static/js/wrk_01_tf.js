function funcao_valida_salvamento_tarefa_mover_arquivo(){
    let txt_dir_origem = document.getElementById('txt_dir_origem');
    let txt_dir_destino = document.getElementById('txt_dir_destino');
    let txt_select_acao = document.getElementById('txt_select_acao');
    let txt_ini_arq = document.getElementById('txt_ini_arq');
    let txt_fim_arq = document.getElementById('txt_fim_arq');
    ...
    if(txt_dir_origem.value == '' 
        || txt_dir_destino.value == ''
        || txt_select_acao.value == ''
        || txt_ini_arq.value == ''
        || txt_fim_arq.value == ''
        || txt_hr_exe.value == ''
        || txt_hr_limit.value == ''
      ...
      if(txt_recorrencia.value == '3' &  txt_dia_mes.value == ''){
        ...
        if(valida_padrao_dependencia(txt_depend_tar.value) === false){
            alert('A configuração de dependencia está fora do padrão.');
            return;
        }

        let pagina = "salvar_tf";
        $.ajax({
            type: 'GET',
            dataType: 'text',
            url: pagina,
            ...
            data: {
                ...
                txt_dir_destino: txt_dir_destino.value,
                txt_select_acao: txt_select_acao.value,
                txt_ini_arq: txt_ini_arq.value,
                txt_fim_arq: txt_fim_arq.value,
...
function funcao_limpa_campos(){
    ...
    document.getElementById('txt_dir_destino').value='';
    document.getElementById('txt_ini_arq').value='';
    document.getElementById('txt_fim_arq').value='';
...
function funcao_listar_tarefas_tf(){
    let pagina = "listar_tarefas_tf";
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: pagina,
        ...
        success: function (data){
            let tableHtml = '';
            tableHtml += "<table border='1' id='tbl_1' class='tabela'>";
            tableHtml += "<thead><tr>";
            tableHtml += "<td><strong>Id</strong></td>";
            tableHtml += "<td><strong>Desc</strong></td>";
            tableHtml += "<td><strong>Resp</strong></td>";
            tableHtml += "<td><strong>Orig</strong></td>";
            tableHtml += "<td><strong>Dest</strong></td>";
            tableHtml += "<td><strong>Ação</strong></td>";
            tableHtml += "<td><strong>Arq</strong></td>";
            tableHtml += "<td><strong>Ext</strong></td>";
            tableHtml += "<td><strong>Ini</strong></td>";
            tableHtml += "<td><strong>Fim</strong></td>";
            ...
            tableHtml += "<td><strong>Remov</strong></td>";
            ...
            if (data.length > 0){
                ...
                data.forEach(function(dados){
                    if (cadeia !== dados['cadeia']){
                        ...
                        tableHtml += "<td colspan='24' style='border: 2px; border-style: solid dashed; border-color: #09F; text-align: center; white-space: nowrap;'>";
                    ...
                    tableHtml += "<td style='cursor: pointer;' onclick=\"funcao_atualizar_linha("+ dados['id_tarefa'] + ",14);\">" + dados['id_tarefa'] + "</td>";
                    ...
                    tableHtml += "<td><p style='font-size:5px'>" + dados['dir_destino'] + "</p></td>";
                    tableHtml += "<td>" + dados['acao'] + "</td>";
                    tableHtml += "<td>" + dados['ini_arq'] + "</td>";
                    tableHtml += "<td>" + dados['extensao_arq'] + "</td>";
                    ...
                    tableHtml += "<td><span title='Parar'><button type='button' id='" + dados['id_tarefa'] + "' class='btn_tab parar' onclick='if(identificaBtn(this.id,5,1) === true){funcao_atualizar_linha(id,14)};'>Parar</button></span></td>";
                    tableHtml += "<td><span title='Reiniciar'><button type='button' id='" + dados['id_tarefa'] + "' class='btn_tab reiniciar' onclick='if(identificaBtn(this.id,2,1) === true){funcao_atualizar_linha(id,14)};'>Reiniciar</button></span></td>";
