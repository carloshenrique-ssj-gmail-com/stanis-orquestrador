...
def baixar_arquivo_sas(origem=None,destino=None,ini_nome=None,ext=None,acao=None,n_tar=0):
    ...
    try:
        ...
        resultado, lista_arquivos_baixados = vlt.CLASSE_SAS.adm_baixar_arquivos(dir_sas=origem,dir_local=destino,arq_sas=ini_nome,extensao=ext,acao=acao)
        if resultado == '00':
            if len(lista_arquivos_baixados) > 0:
                for arq_baixado in lista_arquivos_baixados:
                    try:
                        dest_arq = str(os.path.join(destino,arq_baixado))
            ...
            msg = f"Falha ao tentar baixar o arquivo do servidor SAS: {str(resultado)}"
            return msg                    

    except Exception as e:
        msg = f"Falha ao tentar baixar o arquivo do servidor SAS: {str(e)}"
        return msg
...
def executa_tarefa():
    ...
    if _tarefa != None and _origem != None and _destino != None and _ini_arq != None and _extensao != None and _acao != None:
        if os.path.exists(_destino):
            ...
            resultado = baixar_arquivo_sas(origem=_origem,destino=_destino,ini_nome=_ini_arq,ext=_extensao,acao=_acao,n_tar=_tarefa)
